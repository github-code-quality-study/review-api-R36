...
# Multiple timestamp formats to match the formats in the data
TIMESTAMP_FORMATS = ['%Y-%m-%dT%H:%M:%S.%fZ', '%Y-%m-%d %H:%M:%S']
...
class ReviewAnalyzerServerTest(unittest.TestCase):
  ...
  def test_get_by_start_date(self):
    ...
    assert all(self.parse_date(result['Timestamp']) >= datetime.strptime(start_date, '%Y-%m-%d') for result in results)
  ...
  def test_get_by_end_date(self):
    ...
    assert all(self.parse_date(result['Timestamp']) <= datetime.strptime(end_date, '%Y-%m-%d') for result in results)
  ...
  def test_get_by_start_and_end_date(self):
    ...
    assert all(self.parse_date(result['Timestamp']) >= datetime.strptime(start_date, '%Y-%m-%d') and self.parse_date(result['Timestamp']) <= datetime.strptime(end_date, '%Y-%m-%d') for result in results)
  ...
  def test_post_generates_a_timestamp(self):
    ...
    timestamp = self.parse_date(result['Timestamp'])
    now = datetime.now()
  ...
  def test_order_reviews_by_sentiment(self):
        ...
        sorted_by_sentiment = sorted(results, key=lambda x: x['sentiment']['compound'], reverse=True)
        self.assertEqual(sorted_by_sentiment[0]['sentiment']['compound'], 0.8)
